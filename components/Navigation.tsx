        ...
        <aside className="hidden lg:flex w-20 fixed h-screen top-0 left-0 border-r border-border-hairline bg-off-white z-50 flex-col items-center py-8 justify-between">
            ...
            <nav className="flex flex-col gap-6 text-[10px] font-bold font-mono text-charcoal-muted">
                ...
                <button
                    ...
                    className={`hover:text-charcoal transition-all flex flex-col items-center gap-1 group ${currentView === 'stitch' ? 'text-charcoal scale-110' : ''}`}
                    title="Stitch Studio"
                >
                    <span className="material-symbols-outlined text-base group-hover:scale-110 transition-transform">design_services</span>
                </button>

                <div className="w-6 border-t border-border-hairline" />
                ...
                {navItems.map((item) => (
                    <a
                        ...
                        className={`hover:text-charcoal transition-all flex flex-col items-center gap-1 group ${currentView === 'dashboard' && activeSection === item ? 'text-charcoal scale-110' : ''}`}
                    >
...
export const Header = ({ onExport, currentView, onNavigate }: HeaderProps) => {
        ...
        <header className="sticky top-0 z-40 bg-off-white/95 backdrop-blur border-b border-border-hairline w-full">
            <div className="px-4 lg:px-16 py-4 flex items-center justify-between">
                ...
                <div className="hidden lg:flex items-center space-x-1 lg:space-x-2 overflow-x-auto no-scrollbar mask-gradient">
                    <button onClick={() => onNavigate('stitch')}>
                        <NavTag text="Stitch Studio" active={currentView === 'stitch'} />
                    </button>
                    <button onClick={() => onNavigate('dashboard')}>
                        <NavTag text="06: Risk & ROI" active={currentView === 'dashboard'} />
                    </button>
                </div>
                ...
                {isMenuOpen && (
                    <div className="absolute top-full left-0 w-full bg-off-white border-b border-border-hairline p-4 lg:hidden shadow-lg flex flex-col gap-2">
                        <MobileNavLink href="#" label="Stitch Studio" onClick={() => { onNavigate('stitch'); setIsMenuOpen(false); }} />
                        <div className="border-t border-border-hairline my-1" />
                        <MobileNavLink href="#01" label="01. Risk Matrix" onClick={() => { onNavigate('dashboard'); setIsMenuOpen(false); }} />
                        <MobileNavLink href="#02" label="02. Metrics" onClick={() => { onNavigate('dashboard'); setIsMenuOpen(false); }} />
                        <MobileNavLink href="#03" label="03. Assumptions" onClick={() => { onNavigate('dashboard'); setIsMenuOpen(false); }} />
                        <MobileNavLink href="#04" label="04. ROI" onClick={() => { onNavigate('dashboard'); setIsMenuOpen(false); }} />
                        <MobileNavLink href="#05" label="05. Team" onClick={() => { onNavigate('dashboard'); setIsMenuOpen(false); }} />
                        <MobileNavLink href="#06" label="06. Milestones" onClick={() => { onNavigate('dashboard'); setIsMenuOpen(false); }} />
                        <MobileNavLink href="#07" label="07. Compliance" onClick={() => { onNavigate('dashboard'); setIsMenuOpen(false); }} />
                        <MobileNavLink href="#08" label="08. Decisions" onClick={() => { onNavigate('dashboard'); setIsMenuOpen(false); }} />
                    </div>
                )}
            </div>
        </header>
